...
from datetime import datetime
from build_history import log_build, get_build_url, create_database
from clone import clone_check  # Import the clone_check function from clone.py
...
class SimpleHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            commit_id = result['repository']['commit_id']  # Extract commit ID from the result
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {'status': 'success', 'build_url': build_url, "test_results": result}
            self.wfile.write(json.dumps(response).encode())
